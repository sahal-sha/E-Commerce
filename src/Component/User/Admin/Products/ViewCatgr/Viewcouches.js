...
const Viewcouches = ({ nav }) => {
  const {item , setItem} = useContext(Mycontext)
  const CouchesItems = item.filter((item) => item.type === 'Couches');
  return (
    <div>
        <IoMdArrowBack onClick={()=>nav('/Products')}/>
      <h3 style={{ color: "sienna" }}>COUCHES</h3>
      <div className='container'>
        <div className='row'>
          {
            CouchesItems.map((item) => (
